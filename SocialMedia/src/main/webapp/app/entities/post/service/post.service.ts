...
type RestOf<T extends IPost | NewPost> = Omit<T, 'createdAt'> & {
...
export class PostService {
  ...
  protected convertDateFromClient<T extends IPost | NewPost | PartialUpdatePost>(post: T): RestOf<T> {
    return {
      ...post,
      createdAt: post.createdAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPost: RestPost): IPost {
    return {
      ...restPost,
      createdAt: restPost.createdAt ? dayjs(restPost.createdAt) : undefined,
    };
  }
