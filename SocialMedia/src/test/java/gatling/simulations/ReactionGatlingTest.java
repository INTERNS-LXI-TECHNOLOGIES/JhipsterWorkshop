 ...
 */
public class ReactionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headers_http).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all reactions").get("/api/reactions").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new reaction")
                        .post("/api/reactions")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"time\": \"2020-01-01T00:00:00.000Z\"" + ", \"reactionMode\": \"LIKE\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_reaction_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created reaction").get("${new_reaction_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created reaction").delete("${new_reaction_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Reaction entity").exec(scn);
